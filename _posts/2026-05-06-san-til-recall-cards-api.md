---
title: "[SAN] TIL 리콜 카드 API: threshold 기반 전체 반환과 Lazy 로딩"
...
먼저 LAZY 로딩을 짚고 넘어가면, JPA에서 연관 엔티티를 **실제로 사용할 때까지 DB 조회를 미루는** 방식입니다.
...
`KnowledgeCard`를 조회할 때 `category`는 즉시 SELECT하지 않고 프록시 객체로 채워뒀다가, `card.getCategory()`를 처음 호출하는 순간 쿼리가 실행됩니다. 반대 개념인 EAGER는 항상 JOIN으로 한 번에 가져오는데, 불필요한 데이터를 항상 로딩하는 성능 문제 때문에 JPA는 기본적으로 LAZY를 권장합니다.

문제는 LAZY 로딩이 실행되려면 그 시점에 트랜잭션(영속성 컨텍스트)이 살아 있어야 한다는 점입니다. `findRelatedByTil()` 내부 트랜잭션은 해당 메서드가 끝나면 종료됩니다. 반환된 `KnowledgeCard` 엔티티는 이미 트랜잭션 밖에 있는 상태고, 이후 `KnowledgeCardResponse.from(card)` 안에서 `card.getCategory()`에 접근하면 Hibernate가 프록시를 초기화하지 못하고 `LazyInitializationException`을 던집니다.
...
> OSIV(Open Session In View)가 활성화된 환경에서는 HTTP 요청 범위 내내 세션이 열려 있어 증상이 숨겨질 수 있습니다. 하지만 OSIV 비활성화 환경이나 서비스 단독 테스트에서는 예외가 터집니다.
