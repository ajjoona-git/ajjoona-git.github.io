...
def get_next_image_index(image_dir, base_name):
    """
    'assets/img/posts/' 폴더를 스캔하여 'base_name_NUMBER.ext' 형식의
    파일을 찾아 가장 큰 NUMBER를 찾고, 그 다음 숫자를 반환합니다.
    (예: 'my-post_1.png', 'my-post_2.png'가 있으면 3을 반환)
    """
    max_index = 0
    # 정규식: '베이스이름_숫자.확장자'
    # re.escape()가 base_name의 '[', ']' 같은 특수문자를 올바르게 처리함
    pattern = re.compile(rf"^{re.escape(base_name)}_(\d+)\..*$")

    if not image_dir.exists():
        return 1 # 'images' 폴더가 없으면 1부터 시작

    # [수정] glob은 모든 파일을 찾고, 정규식이 정확하게 필터링하도록 변경
    # (base_name에 '[', ']' 등이 있어도 안전하게 작동)
...
def process_single_markdown_file(md_file_path, root_dir):
    ...
    # 1. [변경] 최종 목적지 'assets/img/posts/' 폴더 (루트)
    final_image_dir = root_dir / "assets" / "img" / "posts"
    final_image_dir.mkdir(parents=True, exist_ok=True)
    
    # 파일명 (날짜 포함)을 이미지 base 이름으로 사용. 예: '2025-10-00-test-post'
    ...
    # 2. [변경] 정규식: Notion에서 내보낸 '로컬' 이미지 링크만 찾기
    # (http, https, 기존 경로(images/, assets/img/posts/ 등)는 제외)
    ...
    # --- [수정된 핵심 로직] ---
    # 3. 'assets/img/posts' 폴더를 스캔하여 다음 번호(예: 1)를 가져옴
    ...
    for match in matches:
        ...
        try:
            ...
            src_image_path = md_file_path.parent / original_local_path # 예: _posts/image 1.png
            ...
            # 새 이름 생성 (예: 2025-10-00-test-post_1.png)
            new_image_name = f"{base_file_name}_{image_counter}{file_ext}"

            # 최종 저장 위치 (예: assets/img/posts/2025-10-00-test-post_1.png)
            absolute_dest_path = final_image_dir / new_image_name

            # [변경] md파일에 기록될 '루트 기준' 절대 경로 (예: /assets/img/posts/...)
...
if __name__ == "__main__":
    root_directory = Path.cwd() # 예: /path/to/ajjoona-git.github.io
